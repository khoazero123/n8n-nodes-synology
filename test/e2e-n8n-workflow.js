...
async function createWorkflow(credential) {
	...
	const nodes = [
		...
		{ name: 'Create Note Full', type: 'CUSTOM.synologyNoteStation', typeVersion: 1, position: [480, 0], parameters: { resource: 'note', operation: 'create', parentId: "={{ $('Create Notebook').item.json.object_id }}", title: `n8n note E2E ${unique}`, content: `<div>n8n full note E2E ${unique}</div>`, brief: `n8n full note E2E ${unique}`, returnFullNote: true }, credentials: { synologyApi: { id: credential.id, name: credential.name } } },
		{ name: 'Set Public Share', type: 'CUSTOM.synologyNoteStation', typeVersion: 1, position: [720, 0], parameters: { resource: 'share', operation: 'setPublic', shareObjectId: "={{ $('Create Note Full').item.json.object_id }}", permission: 'ro' }, credentials: { synologyApi: { id: credential.id, name: credential.name } } },
		{ name: 'Get Public Link', type: 'CUSTOM.synologyNoteStation', typeVersion: 1, position: [960, 0], parameters: { resource: 'share', operation: 'getPublicLink', shareObjectId: "={{ $('Create Note Full').item.json.object_id }}" }, credentials: { synologyApi: { id: credential.id, name: credential.name } } },
		{ name: 'Delete Public Share', type: 'CUSTOM.synologyNoteStation', typeVersion: 1, position: [1200, 0], parameters: { resource: 'share', operation: 'deletePublic', shareObjectId: "={{ $('Create Note Full').item.json.object_id }}" }, credentials: { synologyApi: { id: credential.id, name: credential.name } } },
		{ name: 'Delete Notebook', type: 'CUSTOM.synologyNoteStation', typeVersion: 1, position: [1440, 0], parameters: { resource: 'notebook', operation: 'delete', objectId: "={{ $('Create Notebook').item.json.object_id }}", recursive: true }, credentials: { synologyApi: { id: credential.id, name: credential.name } } },
	];
	const connections = {};
