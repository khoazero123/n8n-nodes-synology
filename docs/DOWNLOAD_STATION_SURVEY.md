...
|---|--------|----------|-------------|
| 1 | **V1 `create` có hoạt động trên DSM7 không?** | ✅ VERIFIED | **Có, hoạt động** — lỗi 403 trước đó do destination mặc định `home/Drive/Download` không tồn tại. Sau khi tạo folder đó, V1 create URL thành công (`{"success":true}`) và task download bình thường. |
| 2 | **V2 API có trả về "Preserve for other purpose" không?** | ✅ VERIFIED | `SYNO.DownloadStation2.Task` tồn tại (path=`entry.cgi`, min=1, max=2, requestFormat=JSON). Dùng thực tế được cho torrent upload. |
...
| 5 | **`create_list` param trong V2** | ✅ VERIFIED | `create_list=true` → response `data.list_id=["btdlXXXX"]`, `task_id=[]` (UI mở dialog chọn file, task tạo sau khi confirm). `create_list=false` → `data.task_id=["dbid_XXXX"]` trực tiếp. |
...
| 7 | **`SYNO.DownloadStation.Task.List` endpoint** | ✅ VERIFIED | `SYNO.DownloadStation2.Task.List` tồn tại (path=`entry.cgi`, min=1, max=2, methods get/download/delete). Dùng để lấy file list của task/list. |
