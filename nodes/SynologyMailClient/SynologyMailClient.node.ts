...
export class SynologyMailClient implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			// --- Trigger settings (used when this node is the workflow trigger) ---
			{
				displayName: 'Trigger Settings',
				name: 'triggerNotice',
				type: 'notice',
				default: '',
				description: 'When this node is used as the workflow trigger (no incoming connection), it polls the selected mailbox and emits new email threads',
			},
			{
				displayName: 'Mailbox (Trigger)',
				name: 'triggerMailbox',
				type: 'options',
				options: [
					{ name: 'Archived', value: 'archived' },
			...
			{
				displayName: 'Search Keyword (Trigger)',
				name: 'triggerKeyword',
				type: 'string',
				default: '',
			...
			{
				displayName: 'From (Sender) (Trigger)',
				name: 'triggerFrom',
				type: 'string',
				default: '',
	...
	async poll(this: IPollFunctions): Promise<INodeExecutionData[][] | null> {
		...
		const mailbox = this.getNodeParameter('triggerMailbox', 'inbox') as string;
		const keyword = this.getNodeParameter('triggerKeyword', '') as string;
		const from = this.getNodeParameter('triggerFrom', '') as string;
