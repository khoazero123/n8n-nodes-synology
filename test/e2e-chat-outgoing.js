...
const { pass: e2ePass, fail: e2eFail, detail } = require('./n8nE2eLog');
...
(async () => {
	...
	else if (n?.json?.user_id) { owUserId = n.json.user_id; ok('OW create'); }
	else bad('OW create', new Error('unexpected response'));
	...
	if (owUserId) {
		...
		if (found) ok('OW list');
		else bad('OW list', new Error('outgoing webhook not found'));
	}
	...
	if (owUserId) {
		...
		const n3 = s3['Get OW'];
		if (n3?.error || !n3?.json?.token) bad('OW get', new Error(n3?.error || 'token missing'));
		else ok('OW get');
	}
